...
//! ```
//!
//! The path is *not* embedded in the ticket; it is bound via the HMAC, so a
//! ticket for one path cannot be replayed for another.
...
impl TicketSigner {
    ...
    /// Issues a ticket for `path` that expires after `ttl_secs` seconds.
    ///
    /// Returns the opaque ticket string `"rt1.{exp}.{hmac_hex}"`.
    pub fn issue(&self, path: &str, ttl_secs: u64) -> String {
        let exp = now_epoch_secs().saturating_add(ttl_secs);
        let mac_hex = compute_hmac_hex(&self.key, path, exp);
        format!("rt1.{exp}.{mac_hex}")
    }

    /// Verifies that `ticket` was issued for `expected_path` and has not expired.
    ///
    /// Returns `Ok(())` on success, or a [`TicketError`] describing the failure.
    pub fn verify(&self, ticket: &str, expected_path: &str) -> Result<(), TicketError> {
        let parts: Vec<&str> = ticket.splitn(3, '.').collect();
        if parts.len() != 3 || parts[0] != "rt1" {
            return Err(TicketError::Malformed);
        }

        let exp: u64 = parts[1].parse().map_err(|_| TicketError::Malformed)?;

        if exp <= now_epoch_secs() {
            return Err(TicketError::Expired);
        }

        // Reject wrong-length hex early — a valid HMAC-SHA256 hex is always 64 chars.
        // This prevents DoS via arbitrarily large allocations proportional to input.
        if parts[2].len() != 64 {
            return Err(TicketError::Malformed);
        }
        ...
        let expected_bytes =
            hex::decode_hmac_input(parts[2]).ok_or(TicketError::InvalidSignature)?;

        let mut mac =
            HmacSha256::new_from_slice(&self.key).map_err(|_| TicketError::InvalidSignature)?;
        mac.update(hmac_message(expected_path, exp).as_bytes());
...
fn hmac_message(path: &str, exp: u64) -> String {
    format!("rt1\0{path}\0{exp}")
}

/// Computes `HMAC-SHA256(key, message)` and returns the lowercase hex digest.
fn compute_hmac_hex(key: &[u8; 32], path: &str, exp: u64) -> String {
    ...
    mac.update(hmac_message(path, exp).as_bytes());
...
mod tests {
    ...
    fn test_issue_verify_roundtrip() {
        ...
        let ticket = signer.issue(path, DEFAULT_TICKET_TTL_SECS);
        assert!(
            ticket.starts_with("rt1."),
            "ticket must start with version prefix"
        );
        assert_eq!(
            ticket.splitn(3, '.').count(),
            3,
            "ticket must have 3 dot-separated parts"
        );
        signer
            .verify(&ticket, path)
    ...
    fn test_wrong_path_fails() {
        let signer = TicketSigner::new();
        let ticket = signer.issue("/etc/passwd", DEFAULT_TICKET_TTL_SECS);
    ...
    fn test_expired_ticket() {
        ...
        let ticket = signer.issue("/tmp/file", 0);
    ...
    fn test_malformed_ticket() {
        let signer = TicketSigner::new();
        for bad in &["", "rt1", "rt1.abc", "rt2.12345.aabb", "notrt1.12345.aabb"] {
    ...
    fn test_tampered_signature() {
        ...
        let mut parts: Vec<&str> = ticket.splitn(3, '.').collect();
        let sig = parts[2];
