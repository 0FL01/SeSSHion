//! SFTP transfer tests using SSH key authentication
#![cfg(unix)]
...
async fn test_sftp_file_put_with_key_auth() {
    ...
    assert!(resp.ok, "sftp file PUT should succeed: {:?}", resp.error);
    assert_eq!(
        resp.transport_used,
    ...
    assert!(verify_text.contains("hello via sftp"));
...
async fn test_sftp_file_get_with_key_auth() {
    ...
    let remote_file = format!("{}/sftp-get-test.txt", remote_home);

    // Create file on remote
    let _ = server
        .test_execute_command(&format!(
            "sh -c 'echo \"sftp file content from remote\" > {}'",
            ssh_mcp::escape_for_shell(&remote_file)
        ))
        .await
    ...
    let local_base = PathBuf::from("target/tmp").join(format!("sftp-get-{unique}"));
    std::fs::create_dir_all(&local_base).expect("create local base");
    ...
    assert!(resp.ok, "sftp file GET should succeed: {:?}", resp.error);
    assert_eq!(
        resp.transport_used,
        TransferTransport::Sftp,
        "should use Sftp transport"
    ...
    assert!(content.contains("sftp file content from remote"));
...
async fn test_sftp_directory_put_with_key_auth() {
    ...
    let local_base = PathBuf::from("target/tmp").join(format!("sftp-dirput-{unique}"));
    std::fs::create_dir_all(&local_base).expect("create local base");
    ...
    std::fs::write(local_dir.join("top.txt"), "top level sftp\n").expect("write top file");

    let local_dir_param = local_dir.to_string_lossy().to_string();
    let remote_dir = format!("{}/sftp_received_dir", remote_home);
    ...
    assert!(
        resp.ok,
        "sftp directory PUT should succeed: {:?}",
        resp.error
    );
    assert_eq!(
        resp.transport_used,
...
async fn test_sftp_directory_get_with_key_auth() {
    ...
    let remote_dir = format!("{}/sftp_source_dir", remote_home);

    // Create remote directory tree
    let _ = server
        .test_execute_command(&format!(
            "sh -c 'mkdir -p {}/subdir && echo \"sftp nested\" > {}/subdir/nested.txt && echo \"sftp top\" > {}/top.txt'",
            ssh_mcp::escape_for_shell(&remote_dir),
            ssh_mcp::escape_for_shell(&remote_dir),
            ssh_mcp::escape_for_shell(&remote_dir)
        ))
    ...
    let local_base = PathBuf::from("target/tmp").join(format!("sftp-dirget-{unique}"));
    std::fs::create_dir_all(&local_base).expect("create local base");
    ...
    assert!(
        resp.ok,
        "sftp directory GET should succeed: {:?}",
        resp.error
    );
    assert_eq!(
        resp.transport_used,
    ...
    assert!(nested_content.contains("sftp nested"));
